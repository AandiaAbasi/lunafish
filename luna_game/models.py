...
class SyncableModel(models.Model):
    ...
    updated_at = models.DateTimeField(_('آخرین تغییر'), auto_now=True, db_index=True)
...
class GameLevel(SyncableModel):
    class Audience(models.TextChoices):
        CHILD = 'child', _('کودک')
        TEEN = 'teen', _('نوجوان')
        ADULT = 'adult', _('بزرگسال')

    code = models.SlugField(_('کد سطح'), max_length=50, unique=True)
    title = models.CharField(_('عنوان سطح'), max_length=100)
    description = models.TextField(_('توضیحات'), blank=True)
    audience = models.CharField(
        _('گروه سنی'),
        max_length=10,
        choices=Audience.choices,
        default=Audience.CHILD,
    )
    order = models.PositiveSmallIntegerField(_('ترتیب'), default=1, db_index=True)

    class Meta:
        ordering = ['order', 'id']
        verbose_name = _('سطح بازی')
        verbose_name_plural = _('سطح‌های بازی')
...
class GameStage(SyncableModel):
    ...
    code = models.SlugField(_('کد مرحله'), max_length=70, unique=True)
    title = models.CharField(_('عنوان مرحله'), max_length=100)
    order = models.PositiveSmallIntegerField(_('ترتیب در سطح'), default=1)
    ...
    class Meta:
        ...
        verbose_name = _('مرحله بازی')
        verbose_name_plural = _('مرحله‌های بازی')
        constraints = [
    ...
    def clean(self):
        ...
        if self.min_difficulty > self.max_difficulty:
            raise ValidationError({
                'max_difficulty': _('حداکثر سختی نمی‌تواند از حداقل سختی کمتر باشد.'),
            })
...
class WordTopic(SyncableModel):
    code = models.SlugField(_('کد موضوع'), max_length=50, unique=True)
    title = models.CharField(_('عنوان موضوع'), max_length=100)
    order = models.PositiveSmallIntegerField(_('ترتیب'), default=1)

    class Meta:
        ordering = ['order', 'title', 'id']
        verbose_name = _('موضوع کلمه')
        verbose_name_plural = _('موضوعات کلمات')
...
class WordPair(SyncableModel):
    ...
    en = models.CharField(_('کلمه انگلیسی'), max_length=150)
    fa = models.CharField(_('معنی فارسی'), max_length=150)
    ...
    admin_note = models.CharField(_('یادداشت ادمین'), max_length=255, blank=True)

    class Meta:
        ordering = ['level__order', 'difficulty', 'id']
        verbose_name = _('جفت کلمه')
        verbose_name_plural = _('جفت‌های کلمه')
        constraints = [
    ...
    def clean(self):
        ...
        if not self.en:
            raise ValidationError({'en': _('کلمه انگلیسی الزامی است.')})
        if not self.fa:
            raise ValidationError({'fa': _('معنی فارسی الزامی است.')})
