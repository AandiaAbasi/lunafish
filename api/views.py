...
class GetChatHistoryAPIView(APIView):
    """
    دریافت تاریخچه پیام‌های یک اتاق چت
    
    - فقط شرکت‌کنندگان دسترسی دارند
    - پیام‌های حذف‌شده نمایش داده نمی‌شوند
    - از بزرگترین به کوچکترین id (نزولی)
    """
    permission_classes = [IsAuthenticated]
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('chat_room_id', OpenApiTypes.INT, required=True, 
                           location=OpenApiParameter.PATH, description='Chat Room ID')
        ],
    ...
    def get(self, request, chat_room_id):
        ...
        try:
            ...
            return Response(
                {'error': _('اتاق چت یافت نشد')},
                status=status.HTTP_404_NOT_FOUND
            )
        
        # بررسی دسترسی - فقط شرکت‌کنندگان
        if not chat_room.participants.filter(user_id=request.user.id).exists():
            return Response(
                {'error': _('شما دسترسی به این چت ندارید')},
                status=status.HTTP_403_FORBIDDEN
            )
        
        messages = chat_room.messages.filter(is_deleted=False).order_by('-id')
...
class SendMessageAPIView(APIView):
    ...
    - فقط شرکت‌کنندگان می‌توانند پیام بفرستند
    - message_type: text, image, video, audio, pdf, sticker
    - برای message_type='text' فیلد text الزامی است
    - برای سایر نوع‌ها فیلد file الزامی است
    """
    permission_classes = [IsAuthenticated]
    parser_classes = (MultiPartParser, FormParser, JSONParser)
        ...
        request=inline_serializer(
            ...
            fields={
                'message_type': serializers.ChoiceField(
                    choices=['text', 'image', 'video', 'audio', 'pdf', 'sticker']
                ),
    ...
    def post(self, request, chat_room_id):
        ...
        if not message_type:
            return Response(
                {'error': _('message_type الزامی است')},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if message_type == 'text':
            if not text or not text.strip():
                return Response(
                    {'error': _('برای پیام متنی text الزامی است')},
                    status=status.HTTP_400_BAD_REQUEST
                )
        else:
            if not file:
                return Response(
                    {'error': _('برای این نوع پیام file الزامی است')},
                    status=status.HTTP_400_BAD_REQUEST
                )
        ...
        message = Message.objects.create(
            ...
            file=file if message_type != 'text' else None
        )
...
class AddReactionAPIView(APIView):
    ...
    - فقط شرکت‌کنندگان اتاق چت می‌توانند واکنش اضافه کنند
    - reaction_type: like, dislike, heart, clap, star
    - اگر کاربر قبلاً این واکنش را افزوده بود، آن حذف می‌شود (toggle)
    """
    permission_classes = [IsAuthenticated]
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('message_id', OpenApiTypes.INT, required=True, 
                           location=OpenApiParameter.PATH, description='Message ID')
        ],
        request=inline_serializer(
            name='AddReactionRequest',
            fields={
                'reaction_type': serializers.ChoiceField(
                    choices=['like', 'dislike', 'heart', 'clap', 'star']
                ),
            }
        ),
    ...
    def post(self, request, message_id):
        ...
        try:
            ...
            return Response(
                {'error': _('پیام یافت نشد')},
                status=status.HTTP_404_NOT_FOUND
            )
        
        # بررسی دسترسی - فقط شرکت‌کنندگان
        if not message.chat_room.participants.filter(user_id=request.user.id).exists():
            return Response(
                {'error': _('شما دسترسی به این چت ندارید')},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        if not reaction_type:
            return Response(
                {'error': _('reaction_type الزامی است')},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # چک کردن وجود واکنش (toggle)
...
class ListParticipantsAPIView(APIView):
    """
    دریافت لیست شرکت‌کنندگان یک اتاق چت
    ...
    @extend_schema(
        ...
        description='Get list of all participants in a chat room',
        parameters=[
            OpenApiParameter('chat_room_id', OpenApiTypes.INT, required=True, 
                           location=OpenApiParameter.PATH, description='Chat Room ID')
        ],
        responses={200: ChatParticipantSerializer(many=True)}
    )
    def get(self, request, chat_room_id):
