...
def complete_teacher_registration(verification_token: str, username: str, password: str, 
    ...
    try:
        ...
        # Create teacher user with role='teacher' and is_teacher_verified=False
        # Admin must verify the teacher later
        user = User.objects.create(
            username=username,
            phone=stored_phone,
            email=token.email,
            role='teacher',  # Set as teacher immediately
            is_teacher_verified=False,  # But not verified - admin must verify
            teacher_verification_requested_at=timezone.now(),  # Track when registration was requested
